...
namespace WorkMapper.Collections
{
    ...
    internal sealed class ProfileMapperHashArray
    {
        ...
        public ProfileMapperHashArray(int initialSize)
        {
        ...
        private static int CalculateHash(string profile, Type sourceType, Type targetType)
        {
            unchecked
            {
                return profile.GetHashCode() ^ sourceType.GetHashCode() ^ (targetType.GetHashCode() * 397);
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool TryGetValue(string profile, Type sourceType, Type targetType, [MaybeNullWhen(false)] out ObjectMapperInfo? item)
        {
            var temp = nodes;
