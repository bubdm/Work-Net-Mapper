namespace WorkMapper
{
    ...
    public sealed class ImmutableTypePairProfileHashArray<T>
    {
        ...
        public ImmutableTypePairProfileHashArray(ICollection<Tuple<Type, Type, string, T>> source)
        {
            ...
            {
                var node = new Node(entry.Item1, entry.Item2, entry.Item3, entry.Item4);
                UpdateLink(ref nodes[CalculateHash(node.SourceType, node.DestinationType, node.Profile) & (nodes.Length - 1)], node);
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool TryGetValue(Type sourceType, Type targetType, string profile, out T item)
        {
            var temp = nodes;
        ...
        private sealed class Node
        {
            ...
            public Node(Type sourceType, Type targetType, string profile, T item)
            {
